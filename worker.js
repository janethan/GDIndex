...
const HTML = `<!DOCTYPE html><html lang=en><head><meta charset=utf-8><meta http-equiv=X-UA-Compatible content="IE=edge"><meta name=viewport content="width=device-width,initial-scale=1"><title>${self.props.sitename}</title><link href="/~_~_gdindex/resources/css/app.css" rel=stylesheet></head><body><script>window.props = { title: '${self.props.sitename}', id: '${self.props.id}', api: location.protocol + '//' + location.host, upload: ${self.props.upload} }<\/script><div id=app></div><script src="/~_~_gdindex/resources/js/app.js"><\/script></body></html>`;

async function onGet(request) {
  let {
    pathname: path
  } = request;
  const rootId = await gd.getId(request.pathname, self.props.id) || self.props.id; 
  // return new Response(`${await gd.getId( request.pathname, self.props.id)} || ${self.props.id}`, { status: 500 }); // 返回带有错误信息的响应
  if (path.startsWith('/~_~_gdindex/resources/')) {
    ...
    const result = await gd.getMetaByPath(path, self.props.id);
...
async function onPost(request) {
  ...
  if (path.substr(-1) === '/') {
    return new Response(JSON.stringify(await gd.listFolderByPath(path, self.props.id)), {
      headers: {
        'Content-Type': 'application/json'
      }
    });
...
async function handleRequest(request) {
  ...
  if ((self.props.lite ) && request.pathname.endsWith('/')) {
    ...
    const {
      files
    } = await gd.listFolderByPath(path, self.props.id);
    let fileht = '';
