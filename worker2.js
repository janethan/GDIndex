self.props = {
  all_root_id: { // ?default_root_id=root ?default_root_title=xxx
    "root": { // 本行前面的root是网盘根目录，可换成文件夹ID
      ...
      auth: false, //  默认关闭验证；改成`true`启用验证，下面的用户名和密码才会生效
      "user": "", // 用户名
      "pass": "", // 密码
...
class GoogleDrive {
  ...
  async newfolder(parentId, name) {
    await this.initializeClient();
    return await this.client.post('https://www.googleapis.com/drive/v3/files', {
    json: {
        "name": name,
...
//    console.log(createResp,createResp.headers.get('Location'));
//    const putUrl = createResp.headers.get('Location');
//    return this.client.put('https://www.googleapis.com/upload/drive/v3/files', {
//      body: {
//        "name": name,
//        "mimeType": "application/vnd.google-apps.folder",
//            parents: [parentId],
...
//    }).json();
  ...
  async newfolderByPath(path, name, rootId = 'root') {
    const id = await this.getId(path, rootId);
//    console.log(path,name, rootId,id);
    if (!id) return null;
    return this.newfolder(id, name);
  }
  async delete(fileId) {
    return this.client.delete(`https://www.googleapis.com/drive/v3/files/${fileId}`, {qs:{  "supportsAllDrives": true,}});
  }
  ...
  async trash(fileId) {
    return this.client.patch(`https://www.googleapis.com/drive/v3/files/${fileId}`,{
      qs: {
        supportsAllDrives: true
      },
      json: {
  ...
  async trashByPath(path, rootId = 'root') {
    const id = await this.getId(path, rootId);
//    console.log(id,path,rootId);
    if (!id) return null;
  ...
  async renameByPath(path, rootId = 'root',newName="null") {
    const id = await this.getId(path, rootId);
    if (!id) return null;
    if (!newName || newName=="null") return null;
    return this.rename(id,newName);
  }
  async move(fileId,to) {
    const result = await this.getMeta(fileId);
    return this.client.patch(`https://www.googleapis.com/drive/v3/files/${fileId}`,{
      qs: {
        supportsAllDrives: true,
        "addParents": to,
       "removeParents": result["parents"].join(","),
       fields:"id, parents",
      },
//      json: {
////"parents":[to,],
...
//      }
  ...
  async moveByPath( source,to,rootId = 'root',) {
    if (!source || source=="null") return null;
    if (!to || to=="null") return null;
    if (to.indexOf("/")!=-1){
       const id = await this.getId(to, rootId);
        if (!id) return null;
        to = id;
    }
    return this.move(source, to);
  }
...
async function onGet(request) {
  ...
  if (!url.pathname.startsWith('/~_~_gdindex') && !self.props.all_root_id.hasOwnProperty(get__default_root_id(request))) {
...
<body>
Unspecified parameter: default_root_id, default_root_title, rootId
</body>
</html>`;
  ...
  const rootId = await gd.getId(request.pathname, get__default_root_id(request)) || get__default_root_id(request);
  ...
  if (path.startsWith('/~_~_gdindex/resources/')) {
    ...
    const result = await gd.getMetaByPath(path, get__default_root_id(request));
...
async function onPost(request) {
  ...
  let current_dir_id = await gd.getId(request.pathname, get__default_root_id(request));
  if (!current_dir_id){current_dir_id = "null";}
  if (path.substr(-1) === '/') {
    return new Response(JSON.stringify(await gd.listFolderByPath(path, get__default_root_id(request))), {
      headers: {
        'Content-Type': 'application/json',
...
async function onPut(request) {
  if (!get__default_root_attr(get__default_root_id(request)).upload) {
    return new Response("Edit isn't enabled.", {
      headers: {
...
//  }
  ...
  const nfolder = params.get('nfolder');
...
//  console.log(nfolder);
  if (nfolder) {
      ...
      const rootId = await gd.getId(request.pathname, get__default_root_id(request)) || get__default_root_id(request);
      return new Response(JSON.stringify(await gd.newfolderByPath(parent, name, rootId)), {
        headers: {
          'Content-Type': 'application/json'
        }
      });
      ...
      const tok = path.split('/');
      const parent = tok.join('/');
      const rootId = get__default_root_id(request);
      return new Response(JSON.stringify(await gd.renameByPath(parent, rootId, rename)), {
    headers: {
      'Content-Type': 'application/json'
    }
  });
      ...
      return new Response(JSON.stringify(await gd.moveByPath(source, to,get__default_root_id(request))), {
    headers: {
      'Content-Type': 'application/json'
    }
  });
      ...
      const fileTypesArray = get__default_root_attr(get__default_root_id(request)).not_allowed_upload.split(';');
      const filteredFileTypes = fileTypesArray.filter(type => type.trim() !== '');
      for (const ty of filteredFileTypes) {
          if (name.toLowerCase().endsWith("."+ty.toLowerCase())){
            // const fileType = type.replace('*', '\\*');
            // const regex = new RegExp(fileType + '$', 'i');
            // return regex.test(fileName);
             return new Response("The file type isn't allowed.", {
              headers: {
                'Content-Type': 'text/plain'
              },
              status: 405
...
async function onDelete(request) {
...
//  }
  ...
  const url = new URL(request.url);
  const params = new URLSearchParams(url.search);
  const delete_forever = params.get('delete_forever');
  console.log("delete_forever",delete_forever);
  if (delete_forever=="true") {

      if (!get__default_root_attr(get__default_root_id(request)).del_file_forever) {
        return new Response("Delete forever isn't enabled.", {
          headers: {
            'Content-Type': 'text/plain'
          },
          status: 405
      ...
      return new Response(JSON.stringify(await gd.deleteByPath(parent, rootId)), {
        headers: {
          'Content-Type': 'application/json'
        }
      });
     ...
     if (!get__default_root_attr(get__default_root_id(request)).del_file) {
        return new Response("Delete isn't enabled.", {
          headers: {
            'Content-Type': 'text/plain'
          },
          status: 405
    ...
    return new Response(JSON.stringify(await gd.trashByPath(parent, rootId)), {
    headers: {
      'Content-Type': 'application/json'
    }
  });
...
function doBasicAuth(request) {
  ...
  return user === get__default_root_attr(get__default_root_id(request)).user && pass === get__default_root_attr(get__default_root_id(request)).pass;
}
...
function get__default_root_id(request) {
  const url = new URL(request.url);
  const params = new URLSearchParams(url.search);
  const rootId = params.get('rootId');
  const default_root_id = params.get('default_root_id');
  const default_root_title = params.get('default_root_title');
  if (self.props.all_root_id.hasOwnProperty(rootId)) {
     return rootId;
  }
  for (const key in self.props.all_root_id) {
    if (self.props.all_root_id[key].title == default_root_title) {
        return key;
    }
  }
  if (default_root_id == null && Object.keys(self.props.all_root_id).length==1){ // 只有一个id
    return Object.keys(self.props.all_root_id)[0];
  }
  return default_root_id;
}
function get__default_root_attr(default_root_id) {
  if (self.props.all_root_id.hasOwnProperty(default_root_id)) {
    return self.props.all_root_id[default_root_id];
  }
  return {title:"hello",auth:false,upload:false,del_file:false,del_file_forever:false,not_allowed_upload:"",}
}
...
async function handleRequest(request) {
  ...
  if (get__default_root_attr(get__default_root_id(request)).auth && !doBasicAuth(request)) {
  ...
  if ((self.props.lite ) && request.pathname.endsWith('/')) {
    ...
    const {
      files
    } = await gd.listFolderByPath(path, get__default_root_id(request));
    let fileht = '';
...
${fileht}
  ...
  let resp;
  if (request.method === 'GET') resp = await onGet(request);else if (request.method === 'POST') resp = await onPost(request);else if (request.method === 'PUT') resp = await onPut(request);else if (request.method === 'DELETE') resp = await onDelete(request);else resp = new Response('', {
    status: 405
  });
...
addEventListener('fetch', event => {
  ...
  const default_root_id = get__default_root_id(updatedRequest);
//  console.log('default_root_id:', default_root_id,'default_root_attr:', get__default_root_attr(default_root_id));
  HTML = `<!DOCTYPE html><html lang="zh-CN"><head><meta charset=utf-8><meta http-equiv=X-UA-Compatible content="IE=edge"><meta name=viewport content="width=device-width,initial-scale=1"><title>${get__default_root_attr(default_root_id).title}</title><link href="/~_~_gdindex/resources/css/app.css" rel=stylesheet></head><body><script>window.props = { title: '${get__default_root_attr(default_root_id).title}', default_root_id: '${default_root_id}', api: location.protocol + '//' + location.host, upload: ${get__default_root_attr(default_root_id).upload}, del_file: ${get__default_root_attr(default_root_id).del_file}, del_file_forever: ${get__default_root_attr(default_root_id).del_file_forever}, not_allowed_upload: "${get__default_root_attr(default_root_id).not_allowed_upload}", }<\/script><div id=app></div><script src="/~_~_gdindex/resources/js/app.js"><\/script></body></html>`;
